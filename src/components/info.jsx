    ...
    return (
        <div className='bg-white ml-[3rem] w-[450px] h-[500px] mb-[2rem]'>
            <div>
                ...
                <form action='/' method='GET' className='flex flex-col mt-[1rem] gap-12'>
                    {InfoText.map((item, index) => (
                        (index !== 0 &&
                            <div>
                                <label for={item.h3} className='font-thin primary-color--100'>{item.label}</label>
                                <input type={item.type} name={item.h3} id={item.h3} placeholder={item.plHolder} className='border w-full px-4' required />
                            </div>)
                    ))}
                </form>
            </div>
